...
def wt(intcode, source):
    intcode.output_values.insert(0, source.get_input_val(intcode.opcodes))
    return 3
...
# 1st
def try_all(inputs, opcodes):
    prev_output = 0
    for i in inputs:
        machine = Intcode(opcodes, instructions, [i, prev_output])
        prev_output = machine.run_all()
    return prev_output

nums = list(range(5))
print("First part:", max(try_all(x, opcodes) for x in itertools.permutations(nums)))

# 2nd
def try_all_feedback(inputs, opcodes):   
    machines = []
    for i in inputs:
        machines.append(Intcode(opcodes, instructions, [i]))
    curr_machine = 0
    prev_output = 0
    while not all(m.halted for m in machines):
        machine = machines[curr_machine]
        machine.send_input(prev_output)
        prev_output = machine.run_all()
        curr_machine = (curr_machine+1)%len(inputs)
    return machines[-1].output_values[0]

nums = list(range(5, 10))
print("Second part:", max(try_all_feedback(x, opcodes) for x in itertools.permutations(nums)))